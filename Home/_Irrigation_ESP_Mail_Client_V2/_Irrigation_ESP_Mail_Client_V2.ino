...
#define NUM_WIFI 4
struct WifiCred wifiList[] = {
    {"Def_SSID", "Def_Passwd"},
    {"Airtel_tush_9922", "air33477"},
    {"_Naruto_", "WakeUpNow@5"},
    {"ESP32", "12345678"}};

bool retryWifiAfterAnHour = true;
int wifiRetryCount = 2;
String hostname = "ESP32 Irrigation";
const char *apSsid = "AP_ESP32";
...
Button button1 = {26, 0, 0, 15, false};
Button button2 = {25, 0, 0, 15, false};
...
void IRAM_ATTR button1Intp()
{
  if (millis() - button1.lastTrigger > 500)
  {
    ...
    Serial.printf("Button 1 has been pressed at %u \n", button1.lastTrigger);
    processButton1 = true;
...
void setup()
{
  ...
  // When we don't need wifi once we get time from server
...
void initSMTP()
{
  ...
  String msg = "IP: " + IP.toString(); //.c_str();
  char *xyz = &msg[0];
  char *emailBody = "Wifi Connected";
  String result = sendEmail("Wifi Connected", xyz, false);
...
void checkWifi()
{
  ...
  {
    showMsg("Check Wifi " + String(wifiRetryCount));
    initWifi();
    initServer();
...
void performTask(int taskIndex)
{
  ...
  String msg = taskList[taskIndex].title + ": " + taskList[taskIndex].h + ":" + taskList[taskIndex].m;
  ...
  showMsg(taskList[taskIndex].title);
  // Send plain email
  char *xyz = &msg[0];
  String result = sendEmail("Task Completed", xyz, false);
}
...
void loop()
{
  ...
  int triggerDiff = now - button1.lastTrigger;
  if (processButton1 && (triggerDiff > (1 * 1000)))
  {
    String str = "lastTrigger ";
    str += button1.lastTrigger;
    Serial.println(str);
    String str2 = "now ";
    str2 += now;
    Serial.println(str2);
    String str3 = "triggerDiff ";
    str3 += triggerDiff;
    Serial.println(str3);
...
void initWifi()
{
  ...
  for (int i = 0; i < NUM_WIFI; i++)
  {
    if (WiFi.status() != WL_CONNECTED)
    {
      ...
      showMsg("Connecting " + String(wifiList[i].ssid));
  ...
  else if (retryWifiAfterAnHour == true && wifiRetryCount > 0)
  {
...
{
  int maxWait = 15;
  while (!getLocalTime(&timeinfo) && maxWait-- > 0 && WiFi.status() == WL_CONNECTED)
  {
...
void handle_connectWifi()
{
  ...
  if (server.hasArg("ssid") & server.hasArg("password"))
  {
    ...
    wifiList[0].ssid = &s[0];
    wifiList[0].password = &p[0];

    Serial.println(wifiList[0].ssid);
    Serial.println(wifiList[0].password);
    server.send(200, "text/html", SendHTML());
    initWifi();
  }
  else
...
void handle_insertTask()
{
  ...
  if (server.hasArg("t") & server.hasArg("h") & server.hasArg("m") & server.hasArg("r") & server.hasArg("isTurnOn") & server.hasArg("goToSleepSec"))
  {
    ...
    m = server.arg("m").toInt();
    r = server.arg("r").toInt();
    isTurnOn = server.arg("isTurnOn").toInt();
...
void print_wakeup_touchpad()
{
  ...
  case 2:
    ...
    break; // connected to internal led don't use
  case 3:
