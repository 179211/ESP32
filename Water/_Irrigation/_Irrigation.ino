...
#define1 AUTHOR_PASSWORD "Aspirine@123"
...
IPAddress2 local_ip(192,168,1,1);
...
//const char* ssid = "Manisha";  // Enter SSID here
//const char* password = "Manisha12345";  //Enter Password here
...
void setup(){
  ...
  data.setLogin(SMTP_HOST, SMTP_PORT, AUTHOR_EMAIL, AUTHOR_PASSWORD);
  data.setSender("Esp32", AUTHOR_EMAIL);
  data.addRecipient(RECIPIENT_EMAIL);

  initTime(); // To set gmtoffset
  initDisplay();
...
//  btStop();
  ...
  //relay starts with 0
  Task newTask1 = {"Task 1", 10, 0, 0, true, 0, true, false};
  Task newTask2 = {"Task 2", 10, 30, 0, false, 82800, true, false}; //82800s = 23H
  Task newTask3 = {"Task 3", 10, 0, 1, false, 0, true, false};
  Task newTask4 = {"Task 4", 10, 30, 1, true, 82800, true, false}; //82800s = 23H
  insertTask(newTask1);
  insertTask(newTask2);
  insertTask(newTask3);
  insertTask(newTask4);
...
void initTime(){
  ...
  Serial.println("Geting time from server");
...
void initWifi(){
  ...
  if (WiFi.status() == WL_CONNECTED){
    ...
    showMsg(msg);
    // Send plain email 
    Serial.println("Send plain email ");
    ...
    //String result = sendEmail("Test", "ESP32", true);
  }
    ...
    Serial.println("");
    Serial.println("WiFi NOT connected. Going to sleep for 20 sec.");
    msg = "WiFi NOT connected. Going to sleep for 20 sec."; 
    showMsg(msg);
    goToSleep(20);
  }
